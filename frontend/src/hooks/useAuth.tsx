...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const login = async (login: string, password: string): Promise<boolean> => {
    try {
      setLoading(true)
      console.log('Intentando conectar con:', import.meta.env.VITE_API_URL || 'http://localhost:8000')
      ...
      if (error.code === 'ECONNREFUSED' || error.code === 'ERR_NETWORK') {
        toast.error('No se puede conectar con el servidor. Verifique que el backend esté ejecutándose en http://localhost:8000')
